...
def getBaseDir():
    ...
    if os.environ.get(environmentVariableName, False):
        ...
        path = str(input("Enter repository path or press Enter to use current working directory (" +
                   currentWorkingDirectory+") : ") or currentWorkingDirectory)
        ...
        while remaining_download_tries > 0:
            try:
                ...
                if err.code == 404:
                    ...
                    print("error downloading pdf on trial no: " +
                          str(2 - remaining_download_tries))
...
def loadJSON(filepath, create=None, verbose=False):
    ...
    if os.path.exists(filepath):
        ...
        if create == "list":
            ...
            raise Exception("creation type \""+create +
                            "\" in loadJSON is not valid")
...
def loadFile(filepath):
    if os.path.exists(filepath):
        ...
        raise Exception(
            "File {filepath} does not exist".format(filepath=filepath))
