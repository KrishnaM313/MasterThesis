...
if __name__ == '__main__':
    ...
    for category in ["climate", "health"]:
        ...
        for i, entry in enumerate(tqdm(df["politicalGroup"])):
            df.loc[i, ["fixedPoliticalGroup"]] = fixParty(entry)
            df.loc[i, ["PoliticalGroupIdeology"]] = getPartyIdeology(entry)
        ...
        for key in ["fixedPoliticalGroup","PoliticalGroupIdeology"]:
            plotGraph(df, category, key, analysisSummaryDir, 
            startYear=2018, dropNA=True, verbose=True)
