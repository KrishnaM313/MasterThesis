...
from tools_data import getFileList
from tools_dataset import BertDataset, getDataSplitIndices
from tools_logging import logValue, logValues
from tools_nn import evaluateModel, evaluateResult
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for i, batch in enumerate(tqdm(trainDataloader)):
            ...
            train_epoch_predictions.append(
                torch.flatten(train_batch_predicted.cpu()))
        ...
        result = evaluateResult(train_epoch_labels, train_epoch_predictions,
                                run, prefix="epoch_train_", demoLimit=demoLimit)
        print(result)
        logValues(run, result)

        # Testing
        test_result = evaluateModel(
            model, testDataloader, device, run, verbose=True, prefix="epoch_test_", demoLimit=demoLimit)
        logValues(run, test_result)

    result = evaluateModel(model, trainDataloader, device, run,
                           verbose=True, demoLimit=demoLimit, prefix="final_train_")
    logValues(run, result)
    result = evaluateModel(model, testDataloader, device, run,
                           verbose=True, demoLimit=demoLimit, prefix="final_test_")
    logValues(run, result)
    result = evaluateModel(model, valDataloader, device, run,
                           verbose=True, demoLimit=demoLimit, prefix="final_validation_")
    logValues(run, result)

    print("Finished Training")

    modelName = "bert_"+args.category+"_"+args.labels+"_train" + \
