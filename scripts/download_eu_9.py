...
xmlDir = os.path.join(downloadBaseFolder, "xml")
...
for term in terms:
  ...
  xmlDownloadState = os.path.join(xmlDir,xmlDownloadStateFilename)

  downloadedFiles = os.listdir(xmlDir)
  ...
  for day in range(delta.days + 1):
    ...
    xmlUrl = "https://www.europarl.europa.eu/doceo/document/CRE-%s-%s-%02d-%02d_EN.xml" % (term["number"], date.year, date.month, date.day)
    xmlFilename = "%s-%02d-%02d.xml" % (date.year, date.month, date.day)
    xmlPath = os.path.join(xmlDir,xmlFilename)

    if xmlFilename not in notAvailableDocuments:
      try:
        downloadedFiles = os.listdir(xmlDir)
      except:
        print("Error downloading")
      print(xmlUrl)
      result = downlodFile(xmlUrl,xmlPath)
      if(result == 1):
        notAvailableDocuments.append(xmlFilename)
