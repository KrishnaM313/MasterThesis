...
def getModelFilename(model_type, category, labels, train_share, test_share, verbose=False):
    modelFilename = model_type+'_'+category+'_'+labels+'_train' + \
        str(round(train_share*100))+'_test'+str(round(test_share*100))+".pkl"
    if verbose:
        ic(modelFilename)
...
def loadModel(models_path, model_type, category, labels, train_share, test_share, verbose=False):
    modelFilename = getModelFilename(
        model_type, category, labels, train_share, test_share, verbose=verbose)
...
if __name__ == '__main__':
    ...
    model = loadModel(args.models_path, args.model_type, args.category,
                      args.labels, args.train_share, args.test_share, verbose=True)

    model.to(device)
    model.eval()
    result = evaluateModel(model, valDataloader, device,
                           run, demoLimit=0, verbose=True, prefix="")
