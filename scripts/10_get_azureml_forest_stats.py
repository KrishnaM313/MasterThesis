...
if __name__ == '__main__':
    ...
    for category in runIDs.keys():
        for labels in runIDs[category].keys():
            ...
            experiment = Experiment(ws,"Forest_{}_{}".format(category,labels))
            ...
            result["accuracy"] = {}
            for stage in stages:
                result["accuracy"][stage] = run.get_metrics("final_"+stage+"_accuracy")
            ...
            #print(result)
            saveJSON(result, os.path.join(plotsPath,"forest_{}_{}.json".format(category, labels)))
