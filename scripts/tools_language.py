...
def translateMyMemory(text, from_lang, to_lang):
    ...
    if os.environ.get(envVariable) is not None:
        translator = Translator(
            from_lang=from_lang, to_lang=to_lang, email=os.environ.get(envVariable))
        return translator.translate(text)
    else:
        raise SystemExit("Please define environment variable for MyMemory translation provider: {envVariable}".format(
            envVariable=envVariable))


def translateGoogle(text, from_lang, to_lang):
    translator = googletrans.Translator(raise_exception=True, proxies={
                                        'http': '109.107.200.151:8080'}, use_fallback=True)
...
def translateAzure(text, from_lang, to_lang):
    ...
    if not location_name in os.environ:
        raise Exception(
            'Please set/export the environment variable: {}'.format(location_name))
