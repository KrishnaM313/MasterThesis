...
if __name__ == '__main__':
    ...
    manualFixDBFilepath = os.path.join(
        rootDir, "scripts", "settings", "manual-mep-association.json")
    ...
    keywordsFilePath = os.path.join(
        rootDir, "scripts/02_ingest_transcripts/keywords.json")
    ...
    for i, file in enumerate(files):
        ...
        for n, speech in enumerate(data):
            ...
            if speechHasOnlyNameInfo(speech):
                ...
                print("{i}/{itotal} - {n}/{ntotal} - {file} - mepID is empty for {mepName}".format(
                    i=i, n=n, itotal=len(files), ntotal=len(data), file=date, mepName=data[n]["name"]))
                ...
                for mep in mepsByName:
                    ...
                    if match > 90 and match > best:
                        ...
                        speech['mepid'] = bestID
                        print("Found Name '{mep}' for given name '{name}'. Will now use id '{bestID}'".format(
                            mep=mep, name=name, bestID=bestID))
                if best == 0:
                    print(
                        "No mepID could be found for name: {name}".format(name=name))
            ...
            if "politicalGroup" not in data[n] or data[n]["politicalGroup"] == "":
                if verbose:
                    print("{i}.{n} political group is empty for {mepID}: {mepName}".format(
                        i=i, n=n, mepID=data[n]["mepid"], mepName=data[n]["name"]))
                err, politicalGroup = extractFromName(speech['name'])
                politicalGroup = fixParty(politicalGroup)
                if err is None:
                    if verbose:
                        print("{i}/{itotal} - {n}/{ntotal} - {file} - political group found as reference in name: '{politicalGroup}'".format(
                            i=i, n=n, itotal=len(files), ntotal=len(data), file=date, politicalGroup=politicalGroup))
                    data[n]["politicalGroup"] = politicalGroup
                else:
                    print(err)
                    print("{i}/{itotal} - {n}/{ntotal} - {file} - political group not referenced in Name".format(
                        i=i, n=n, itotal=len(files), ntotal=len(data), file=date))
                    err, mep = findMEP(
                        mepsByID, speech['mepid'], mepInfoDir, mepInfoDBFilepath)
                    if err is not None:
                        print(err)
                    else:
                        ...
                        if err is None:
                            if verbose:
                                print("{i}/{itotal} - {n}/{ntotal} - {file} - political group found in Database: '{politicalGroup}'".format(
                                    i=i, n=n, itotal=len(files), ntotal=len(data), file=date, politicalGroup=party))
                            data[n]["politicalGroup"] = party
                        else:
                            print(err)

            if "politicalGroup" in data[n]:
                data[n]["partyIdeology"] = getPartyIdeology(
                    data[n]["politicalGroup"])
            else:
            ...
            if not (speech['mepid'] == "" or speech['mepid'] == "n/a"):
                ...
                if err is not None:
                    ...
                    if err is not None:
                        ...
                        if verbose:
                            print("{i}/{itotal} - {n}/{ntotal} - {file} - name found in Database for mepID: '{name}'".format(
                                i=i, n=n, itotal=len(files), ntotal=len(data), file=date, name=name))
                        data[n]["name"] = name
